...
async function menu_create(req: Request, res: Response) {
    ...
    const [slug, productName, price, image, in_stock, description, category] =
        wis_array(req);
...
async function menu_modify(req: Request, res: Response) {
    ...
    if (!menu) {
        return res.status(404).json(ErrorFormat(iwe_strings.Menu.ENOTFOUND));
    }
